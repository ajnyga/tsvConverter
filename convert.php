...
$opts = getopt($shortOpts, $longOpts, $rest_index);
...
$onlyValidate = 0;
if (isset($opts['v']) || isset($opts['validate'])) {
...
$defaultLocale = 'en_US';
if (isset($opts['l'])) {
	echo "Default locale is set with value: " . $opts['l'] . "\n";
	$defaultLocale = $opts['l'];
} elseif (isset($opts['defaultLocale'])) {
	echo "Default locale is set with value: " . $opts['defaultLocale'] . "\n";
...
$issueKeys = getUniqueKeys($articles, 'issue');
$sectionKeys = getUniqueKeys($articles, 'section');
$articleKeys = array_diff(getUniqueKeys($articles), $issueKeys, $sectionKeys);
// $sections = [];
...
foreach ($articles as $id => &$article) {
	$oldSections[$article['issueDatepublished']][$article['sectionAbbrev']] = $article['sectionTitle'];

	// identify issue and sections for each article by hash generated from issue and section fields
	$issueIdentification = array_intersect_key($article, array_flip($issueKeys));
	$article['articleIssueHash'] = hash("sha256", implode(", ", $issueIdentification));

	$sectionIdentification = array_intersect_key($article, array_flip($sectionKeys));
	$article['articleSectionHash'] = hash("sha256", implode(", ", $sectionIdentification));

	// $sections[$article['articleSectionHash']] = $sectionIdentification;
	// $sections[$article['articleSectionHash']]['issueIdentificationHash'] = $article['articleIssueHash'];
	...
	$issueData[$article['articleIssueHash']]['articles'][$id] = $article;
}
...
//  Create article DOMs
$tempDOMContainer = $dom->createDocumentFragment();
foreach ($articles as $id => $article) {

	$articleDOM = $dom->createElement('article');
	$id = $dom->createElement('id', $id);
	$id->setAttribute('type', 'internal');
	$id->setAttribute('advice', 'ignore');
	$tempDOMContainer->appendChild($id);
...
foreach ($issueData as $issueHash => $issueDataContent) {
	$issuesDOM = processData($issuesDOM, $issueDataContent);
}
...
function processData($dom, $data) {
	foreach ($data as $tagName => $content) {
		...
		switch ($tagName) {
			case 'sections':
				$sectionsDOM = $dom->ownerDocument->getElementById('sections');
				if ($sectionsDOM->length == 0) {
					$sectionsDOM = $dom->ownerDocument->createElement('sections');
					$dom->lastChild->appendChild($sectionsDOM);
				}
				$sectionDOM = $dom->ownerDocument->createElement('section');
				$sectionsDOM->appendChild($sectionDOM);

				$sectionDOM = processData($sectionDOM, $data['sections']['section']);
			...
			case 'issue_identification':
				...
				$issuesIdentificationDOM = processData($issuesIdentificationDOM, $data[$tagName]);
				break;
			case 'sectionTitle':
			case 'sectionAbbrev':
			...
			case 'issueTitle':
				...
				break;
			case 'articles':
				$articlesDOM = $dom->ownerDocument->createElementNS('http://pkp.sfu.ca', 'articles');
				...
				$dom->lastChild->appendChild($articlesDOM);
				break;
		}
	}
	return $dom;
...
foreach ($articles as $key => $article) {
	...
	if ($currentIssueDatepublished != $article['issueDatepublished']) {
		...
		foreach ($oldSections[$article['issueDatepublished']] as $sectionAbbrev => $sectionTitle) {
